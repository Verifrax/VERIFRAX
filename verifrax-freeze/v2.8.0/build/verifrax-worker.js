...
`;
      return withHeaders(new Response(robotsTxt, {
        status: 200,
        headers: { "Content-Type": "text/plain; charset=utf-8" }
      }));
    }

    // GET /.well-known/verifrax.json (MACHINE DISCOVERY)
    if (path === "/.well-known/verifrax.json" && request.method === "GET") {
      const discovery = {
        system_name: "VERIFRAX",
        version: "2.8.0",
        role: "deterministic_verification_system",
        execution_model: "one_payment_one_execution_one_certificate",
        finality: "absolute",
        reexecution: "forbidden",
        authority_source: "cryptographic_certificate",
        certificate_endpoints: {
          base_url: "https://www.verifrax.net",
          certificate_html: "/certificate/{hash}",
          certificate_json: "/certificate/{hash}.json",
          certificate_proof: "/certificate/{hash}.proof",
          certificate_llm: "/certificate/{hash}.llm"
        },
        machine_readable: true,
        certificate_format: {
          type: "canonical_json",
          hash_algorithm: "SHA-256",
          immutability_proof: "hash_of_hash",
          fields: [
            "bundle_hash",
            "certificate_version",
            "executed_at",
        ...
        protocol_properties: {
          ...
          payment_independent_post_issuance: true,
          operator_independent_post_issuance: true,
      ...
      return withHeaders(new Response(JSON.stringify(discovery, null, 2), {
        status: 200,
        headers: { "Content-Type": "application/json; charset=utf-8" }
      }));
    }
